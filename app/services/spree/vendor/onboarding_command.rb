  ...
  module Vendor
    class OnboardingCommand
      ...
      def call
        ...
        existing = Spree::VendorCommandRecord.find_by(
          vendor_id: @vendor.id,
        ...
        response = ActiveRecord::Base.transaction do
          resource =
            ...
            when "activate_store" then activate_store!
              ...
              raise ArgumentError, "Unsupported vendor command: #{@command_type}"
            end
      ...
      def activate_store!
        ...
        raise ArgumentError, "store must have a geo policy or explicit policy" if store.policy.blank? && !store.geo_configured?
